...
vec3 dither_ok(vec2 uv, vec3 col) {
    float m = getMat2(uv);
    vec3 luma = vec3(0.299, 0.587, 0.114);
    float grayscale = dot(col, luma);
    //vec3 labbed = oklab_from_srgb(col);
    vec3 labbed = oklab_from_linear(col);
    //return linear_from_oklab ( vec3(
    return linear_from_oklab( vec3(
         ditherl_c(2.0,uv,labbed.r),
         ditherl_c(36.0,uv,labbed.g),
         ditherl_c(3.0,uv,labbed.b)
    ));
}
vec3 dither(vec2 uv, vec3 col){
  vec3 okd = dither_ok(uv, col);
  return okd;
  okd = col;
    return ( vec3(
         ditherl(uv,okd.r),
